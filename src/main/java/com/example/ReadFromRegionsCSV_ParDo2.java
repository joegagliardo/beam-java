...
public class ReadFromRegionsCSV_ParDo2 {
	...
	public static PCollection<String> buildPipeline(Pipeline pipeline, String inputFile, String outputFolder) {
		PCollection<String> regions = pipeline
				.apply("Read", TextIO.read().from(inputFile))
				.apply("Parse", ParDo.of(new AddStar()));
		regions.apply(TextIO.write().to(outputFolder));
		return regions;
	}
	...
	public static void main(String[] args) {
		...
		ReadFromRegionsCSV_ParDo2.buildPipeline(pipeline, inputFile, outputFolder);
...
// gradle run -PmainClass="com.example.ReadFromRegionsCSV_ParDo2" --args="--inputFile=data/regions.csv --outputFolder=tmp/regionsCSV_ParDo"

// gradle run -PmainClass="com.example.ReadFromRegionsCSV_ParDo2" --args="--inputFile=gs://joey-dataflow-bucket/data/regions.csv --outputFolder=gs://joey-dataflow-bucket/temp/regions_ParDo"

// gradle run -PmainClass="com.example.ReadFromRegionsCSV_ParDo2" --args="--inputFile=gs://joey-dataflow-bucket/data/regions.csv --outputFolder=gs://joey-dataflow-bucket/temp/output/regions_ParDo --runner=DataflowRunner --project=joey-gagliardo --region=us-central1 --gcpTempLocation=gs://joey-dataflow-bucket/temp --dataflowServiceOptions=enable_prime"
